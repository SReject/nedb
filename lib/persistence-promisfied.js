...
const storage = require('./storage-promisfied');
...
module.exports = class Persistence {
    ...
    persistCachedDatabase(callback = () => {}) {
        ...
        storage.crashSafeWriteFile(this.filename, toPersist)
            .then(() => {
    ...
    persistNewState(newDocs, callback = () => {}) {
        ...
        storage.appendFile(self.filanem, toPersist, 'utf8')
            .then(callback)
            .catch(callback);
    }
    ...
    static ensureDirectoryExists(dir, cb) {
        cb = cb || (() => {});
        storage.mkdirp(dir).then(cb).catch(cb);
    }
