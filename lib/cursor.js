...
// alias methods from model
const match = model.match;
const getDotValue = model.getDotValue;
const compareThings = model.compareThings;
const modify = model.modify;
            ...
            if (self._sort) {
                ...
                let skip,
                    ...
                    criteria = keys.map(key => {
                        return {key: key, direction: self._sort[key]};
                    });

                res.sort((a, b) => {
                    let res = 0;
                    criteria.find(crit => {
                        res = crit.direction * compareThings(getDotValue(a, crit.key), getDotValue(b, crit.key), self.db.compareStrings);
                        return res !== 0;
                    });
                    return res;
                });
