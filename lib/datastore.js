...
module.exports = class Datastore extends Emitter {
    ...
    ensureIndex(options, callback = () => {}) {
        ...
        this.persistence.persistNewState([{ $$indexCreated: options }])
            .then(() => {
                callback(null);
            })
            .catch(callback);
    ...
    removeIndex(fieldName, callback = () => {}) {
        ...
        this.persistence.persistNewState([{ $$indexRemoved: fieldName }])
            .then(() => {
                callback(null);
            })
            .catch(callback);
    ...
    _insert(newDoc, callback = () => {}) {
        ...
        this.persistence
            ...
            .then(() => {
                callback(null, model.deepCopy(preparedDoc));
            }).catch(callback);
    }
    ...
    count(query, callback) {
        let cursor = new Cursor(this, query, (err, docs, callback) => {
            if (err) {
                return callback(err);
            }
     ...
     * @param {Function} [cb]
     */
    _update(query, updateQuery, options, callback = () => {}) {
        let self = this,
            numReplaced = 0,
            multi, upsert,
            i;
        ...
        async.waterfall([
            ...
            () => { // Perform the update
                ...
                self.getCandidates(query, (err, candidates) => {
                    ...
                    self.persistence.persistNewState(updatedDocs)
                        .then(() => {
                            ...
                            callback(null, numReplaced, updatedDocsDC);
                        }).catch(callback);
                });
            }]);
    }
    ...
    */
    ...
    // expose Cursor, Index, Executor and Persistence 'classes'
    static get Cursor() {
        return Cursor;
    }
    static get Index() {
